 ...
 * @author: N3 S.r.l.
 */
class FreetureFinalFactoryBase
{
	public static function Save( $object )
	{
		global $db_conn;
		$ob = clone $object;
		FreetureFinalFactoryBase::cleanData( $ob );
		FreetureFinalFactory::CheckData( $ob );
		$query = "INSERT INTO inaf_freeture_final (`id`,`key`,`value`,`description`,`show`,`create_date`,`valid_from`,`valid_to`,`erased`,`last_update`) VALUES ( $ob->id,$ob->key,$ob->value,$ob->description,$ob->show,$ob->create_date,$ob->valid_from,$ob->valid_to,0,now())";
	...
	{
		global $db_conn;
		$query = "DELETE FROM inaf_freeture_final WHERE id=" . $object->id;
		$res = mysqli_query($db_conn, $query);
	...
	{
		global $db_conn;
		$query = "UPDATE inaf_freeture_final SET erased=1 WHERE id=" . $object->id . " AND erased=0";
		$res = mysqli_query($db_conn,$query);
		if ($res === false) return false; else return true;
	}
	...
	{
		global $db_conn;
		FreetureFinalFactoryBase::cleanData( $object );
		FreetureFinalFactory::CheckData( $object );
		$query = "UPDATE inaf_freeture_final SET `id` = $object->id,`key` =$object->key,`value` =$object->value,`description` =$object->description,`show` = $object->show,`valid_from` = $object->valid_from,`valid_to` = $object->valid_to,`erased` = $object->erased,`last_update`=now() WHERE id=" . $object->id;
		$res = mysqli_query($db_conn,$query);
		if ($res === false) return false; else return true;
	}

	/** @return FreetureFinal */ 
	public static function Get( $id )
	{
		global $db_conn;
		$query = "SELECT * FROM inaf_freeture_final WHERE id=" . $id;
		$res = @mysqli_query($db_conn,$query);
		if ($res === false || mysqli_num_rows($res) <= 0)
			return false;
		$object = mysqli_fetch_object($res,'FreetureFinal');
	...
	public static function GetList($where = '')
	{
		...
		$query = "SELECT * FROM inaf_freeture_final $where_";
		$res = mysqli_query( $db_conn, $query);
		if (!$res  || mysqli_num_rows($res) <= 0)
			return array();
		while ($row = mysqli_fetch_object($res,'FreetureFinal')) {
	...
	public static function getMaxId() {
		global $db_conn;
		$query = "SELECT max(id) as max_id FROM inaf_freeture_final";
		$res = mysqli_query( $db_conn, $query);
		if ($res === false  || mysqli_num_rows($res) <= 0)
			return 1;
		...
		public static function GetListFilter($column,$code,$where = '') {
			...
			$query = "SELECT distinct ".$column." FROM inaf_freeture_final $where_ and ".$column." like '%$code%'; ";
			$res = mysqli_query($db_conn, $query);
			if (!$res || mysqli_num_rows($res) <= 0)
				return array();
		...
		public static function GetForeignKeyParams($column) {
			global $db_conn;
			$object_list = array();
			$query = "SELECT distinct REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE TABLE_NAME = 'inaf_freeture_final' and COLUMN_NAME = '".$column."' limit 1; ";
			$res = mysqli_query($db_conn, $query);
			if (!$res || mysqli_num_rows($res) <= 0)
				return array();
			while ($row = mysqli_fetch_object($res)) {
