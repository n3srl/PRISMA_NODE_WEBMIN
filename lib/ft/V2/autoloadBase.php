...
$dir = $_SERVER['DOCUMENT_ROOT']. "/lib/ft/V2/model";
...
$dir = $_SERVER['DOCUMENT_ROOT']. "/lib/ft/V2/model/view";
if(file_exists($dir)){
	$files_tmp = scandir($dir);
	$files = array();
	foreach ($files_tmp as $key => $value)
...
$dir = $_SERVER['DOCUMENT_ROOT']. "/lib/ft/V2/logic";
$files_tmp = scandir($dir);
$files = array();
foreach ($files_tmp as $key => $value)
{
...
$dir = $_SERVER['DOCUMENT_ROOT']. "/lib/ft/V2/logic/api";
if(file_exists($dir)){
	$files_tmp = scandir($dir);
	$files = array();
	foreach ($files_tmp as $key => $value)
